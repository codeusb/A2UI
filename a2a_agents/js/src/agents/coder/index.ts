...
class CoderAgentExecutor implements AgentExecutor {
    ...
    async execute(
                ...
                status: {
                    ...
                    message: {
                        ...
                        parts: [
                            {
                                kind: "text",
                                text: "No input message found to process.",
                            },
                        ],
                        taskId: taskId,
                        contextId: contextId,
        ...
        try {
            ...
            for await (const chunk of stream) {
                ...
                for (const fileUpdate of codeChunk.files) {
                        ...
                        if (
                            ...
                            const artifactUpdate: TaskArtifactUpdateEvent = {
                                ...
                                artifact: {
                                    ...
                                    name: prevFilename,
                                    parts: [
                                        { kind: "text", text: prevFileContent },
                                    ],
                                },
                    ...
                    if (this.cancelledTasks.has(taskId)) {
                        console.log(
                            `[CoderAgentExecutor] Request cancelled for task: ${taskId}`
                        );
            ...
            const fullMessage = (await response).output as
                | CodeMessage
                | undefined;
            const generatedFiles =
                fullMessage?.files.map((f) => f.filename) ?? [];
